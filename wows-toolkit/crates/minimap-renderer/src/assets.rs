...
/// Whether an image is (almost) entirely the magenta colorkey Lesta uses for
/// placeholder consumable PNGs (~`#FF00D0`). Sampled on a coarse grid.
fn is_magenta_placeholder(img: &image::RgbaImage) -> bool {
    ...
    for y in (0..h).step_by(step_y as usize) {
        for x in (0..w).step_by(step_x as usize) {
            ...
            sampled += 1;
            if a > 200 && r > 230 && g < 40 && b > 160 {
                magenta += 1;
            }
        }
    }
    sampled > 0 && magenta * 100 >= sampled * 90
}

/// Load an image that may be a DDS (Lesta ships real icons as BCn-compressed DDS
/// under `*_dds_scale/` dirs, with the sibling `.png` a solid-magenta placeholder).
/// Falls back to the normal decoder for png/jpeg. Returns an RGBA image.
fn load_image_or_dds_entry(entry: &VfsPath) -> Option<image::RgbaImage> {
    let bytes = read_vfs_entry(entry)?;
    if entry.filename().to_ascii_lowercase().ends_with(".dds") {
        let dds = image_dds::ddsfile::Dds::read(&mut std::io::Cursor::new(&bytes)).ok()?;
        let decoded = image_dds::image_from_dds(&dds, 0).ok()?;
        // Bridge across a possibly-different `image` major bundled by image_dds by
        // rebuilding from raw RGBA8 bytes.
        return image::RgbaImage::from_raw(decoded.width(), decoded.height(), decoded.into_raw());
    }
...
pub fn load_consumable_icons(vfs: &VfsPath, version: Option<&Version>) -> HashMap<String, RgbaImage> {
    ...
    {
        // Keys whose icon we already sourced from a real DDS, so a sibling
        // placeholder PNG doesn't overwrite it.
        let mut dds_keys: std::collections::HashSet<String> = std::collections::HashSet::new();
        for entry in entries {
            let filename = entry.filename();
            let lower = filename.to_ascii_lowercase();
            // WG:    "consumable_PCY009_CrashCrewPremium.png" (real icon)
            // Lesta: same stems under `gui/consumables_dds_scale/`, where the `.png`
            //        is a solid-magenta placeholder and the real icon is the sibling
            //        `.dds`; plus per-scale (`_0`/`_1`) and depleted (`_empty`) variants.
            // Accept both extensions, prefer DDS, skip the `_empty` state, accept the
            // premium `PXY` ids, and strip a trailing `_<scale digit>` so the key
            // matches the base PCY id the renderer looks up.
            let is_dds = lower.ends_with(".dds");
            if !is_dds && !lower.ends_with(".png") {
                continue;
            }
            let stem = &filename[..filename.len() - 4];
            let Some(stem) = stem.strip_prefix("consumable_") else {
                continue;
            };
            if !(stem.starts_with("PCY") || stem.starts_with("PXY")) {
                continue;
            }
            if stem.contains("_empty") {
                continue;
            }
            let key = match stem.rsplit_once('_') {
                Some((base, tail)) if tail.len() == 1 && tail.chars().all(|c| c.is_ascii_digit()) => base,
                _ => stem,
            };
            // Keep an already-loaded icon unless we now have a DDS to upgrade a
            // placeholder PNG with.
            if icons.contains_key(key) && (dds_keys.contains(key) || !is_dds) {
                continue;
            }
            if let Some(img) = load_image_or_dds_entry(&entry) {
                // Some Lesta consumables ship only a solid-magenta colorkey PNG here
                // (the real art lives in the sibling DDS, which isn't in the idx/pkg
                // VFS). Skip those so we draw a real icon or nothing — never a magenta
                // block.
                if is_magenta_placeholder(&img) {
                    continue;
                }
                let resized = image::imageops::resize(&img, 28, 28, image::imageops::FilterType::Lanczos3);
                icons.insert(key.to_string(), resized);
                if is_dds {
                    dds_keys.insert(key.to_string());
                }
            }
        }
    }
...
/// Load a CJK-capable font from a well-known OS location, appended to the fallback
/// chain so Chinese/Japanese/Korean text renders even when the game VFS ships no
/// CJK face. The Russian «Мир кораблей»/Lesta client carries only Cyrillic+Latin
/// Warhelios, so community Chinese ship-name packs (e.g. 泽刻汉化) would otherwise
/// render as tofu boxes.
fn load_system_cjk_font() -> Option<(FontArc, Vec<u8>)> {
    const CANDIDATES: &[&str] = &[
        // Windows
    ...
    for path in CANDIDATES {
        ...
        // .ttc collections need an explicit face index; index 0 is the base face.
        if let Ok(font) = ab_glyph::FontVec::try_from_vec_and_index(bytes.clone(), 0) {
            debug!(path, "Loaded system CJK fallback font");
            return Some((FontArc::from(font), bytes));
        }
    }
    None
}
...
fn game_fonts_from_vfs(vfs: &VfsPath) -> Option<GameFonts> {
    ...
    let mut fallbacks_with_bytes: Vec<(FontArc, Vec<u8>)> =
        fallback_paths.iter().filter_map(|path| load_font(path)).collect();
    // The Russian/Lesta client ships no CJK face; append a system CJK font (lowest
    // priority, so a game-shipped Source Han still wins) so Chinese ship-name packs
    // render as glyphs instead of tofu boxes.
    let has_cjk = fallbacks_with_bytes.iter().any(|(f, _)| f.glyph_id('中').0 != 0);
    if !has_cjk && let Some(cjk) = load_system_cjk_font() {
        fallbacks_with_bytes.push(cjk);
    }
...
fn system_game_fonts() -> GameFonts {
    ...
    let primary_scale_factor = compute_scale_factor(&primary);
    // Latin system faces (Segoe UI/Arial) carry no CJK; append a system CJK font so
    // Chinese/Japanese/Korean text still renders.
    let (fallbacks, fallback_scale_factors, fallback_bytes) = match load_system_cjk_font() {
        Some((font, bytes)) => {
            let sf = compute_scale_factor(&font);
            (vec![font], vec![sf], vec![bytes])
        }
