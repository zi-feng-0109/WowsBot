...
/// of these can't parse replays, so their absence is fatal. (GameParams is handled
/// separately — see GAME_PARAMS_CANDIDATES — because Lesta renames it.)
const REQUIRED_VFS_FILES: &[&str] = &["scripts/entities.xml"];

/// GameParams filename candidates, in priority order. WG ships
/// `content/GameParams.data`; Lesta («Мир кораблей») splits it into
/// `GameParams_py2.data` / `_py3.data`. Extract whichever exists under its own
/// name so the reader (`GameMetadataProvider::from_vfs`, which tries the same
/// three) finds it.
const GAME_PARAMS_CANDIDATES: &[&str] =
    &["content/GameParams.data", "content/GameParams_py2.data", "content/GameParams_py3.data"];
...
pub fn dump_renderer_data(
    ...
    // GameParams: extract the first candidate that exists (WG vs Lesta naming).
    let mut game_params_found = false;
    for cand in GAME_PARAMS_CANDIDATES {
        if extract_vfs_file_cas(&vfs, cand, &vfs_dir, &cas_root, &mut file_hashes)? {
            game_params_found = true;
            break;
        }
    }
    if let Some(pb) = progress {
    ...
    if !game_params_found {
        missing_files.push("content/GameParams.data (or _py2/_py3)");
    }
    let map_count =
...
fn dump_all_translations(game_dir: &Path, build: u32, output_dir: &Path) -> Result<(), Report> {
    let bin_dir = game_dir.join("bin").join(build.to_string());
    // Copy base `res/texts` first, then overlay `res_mods/texts` (mods win for the
    // same lang) — mirroring how the client layers localization. Crucially, Lesta's
    // Chinese (泽刻汉化) ships in `res_mods/texts/ru`, overwriting the base Russian,
    // so the extracted `translations/ru` catalog ends up Chinese, matching an
    // in-client (`-g`) read. No-op overlay for WG installs without res_mods texts.
    let mut copied_any = false;
    for texts_rel in ["res/texts", "res_mods/texts"] {
        let texts_dir = bin_dir.join(texts_rel);
        if !texts_dir.exists() {
            continue;
        }
    ...
    if !copied_any {
        tracing::warn!("No translations found under {}/res(/_mods)/texts", bin_dir.display());
    }
    Ok(())
}
...
fn derive_game_params_rkyv(vfs_dir: &Path) -> Option<Vec<u8>> {
    // WG: content/GameParams.data; Lesta: content/GameParams_py2/_py3.data.
    // `from_vfs` tries all three, so accept any of them here.
    let has_game_params = GAME_PARAMS_CANDIDATES.iter().any(|c| vfs_dir.join(c).exists());
    if !has_game_params {
        return None;
    }
