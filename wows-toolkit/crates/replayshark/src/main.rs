...
where
    F: FnOnce(&wows_replays::ReplayMeta, &[u8]) -> Box<dyn Analyzer>,
{
    ...
    let mut analyzer = build(&replay_file.meta, &replay_file.packet_data);
...
fn survey_file(
    ...
    match parse_replay(&replay, game_dir, extracted_dir, |meta, _packet_data| {
...
fn load_metadata_provider_and_constants(
    ...
    match (game_dir, extracted_dir) {
        ...
        (None, Some(extracted)) => {
            ...
            // Prefer English, but Lesta («Мир кораблей») extractions carry no `en`
            // — only `ru` (which the dump fills with 泽刻汉化 Chinese). Fall back to
            // the first available lang so ship names resolve instead of showing raw ids.
            let translations_dir = dir.join("translations");
            let en_mo = translations_dir.join("en/LC_MESSAGES/global.mo");
            let mo_path = if en_mo.exists() {
                en_mo
            } else {
                std::fs::read_dir(&translations_dir)
                    .ok()
                    .and_then(|rd| {
                        rd.flatten().map(|e| e.path().join("LC_MESSAGES/global.mo")).find(|p| p.exists())
                    })
                    .unwrap_or(en_mo)
            };
            apply_translations(&mo_path, &mut provider);
...
fn main() {
    ...
    match args.command {
        Commands::Dump { output, no_meta, replay } => {
            ...
            parse_replay(&replay, game_dir, extracted, |meta, _packet_data| {
        ...
        Commands::Chat { replay } => {
            parse_replay(&replay, game_dir, extracted, |meta, packet_data| {
                wows_replays::analyzer::chat::ChatLoggerBuilder::new().build(meta, packet_data)
            })
            .unwrap();
        }
