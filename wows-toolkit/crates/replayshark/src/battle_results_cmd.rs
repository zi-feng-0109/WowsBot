...
pub fn run(
    ...
    for path in &replays {
        ...
        if needs_remote_constants && resolver.is_none() {
            match ConstantsResolver::new() {
                Ok(r) => resolver = Some(r),
                // Non-fatal: a replay with no raw results blob (e.g. Lesta, which
                // never embeds one) yields a packet-derived normalized report that
                // needs no constants. `resolve_constants` still errors per-replay if
                // constants are genuinely required (a raw blob is present).
                Err(e) => eprintln!("{stem}: constants resolver unavailable ({e}); proceeding without it"),
            }
        }
...
fn process_replay(
    ...
    let (provider, mut game_constants) = crate::load_metadata_provider_and_constants(game_dir, extracted_dir, &version)
        .map_err(|e| format!("failed to load game data: {e}"))?;
    // Lesta: correct the SPOT/AGRO id swap so spotting/potential aren't reversed.
    game_constants.apply_lesta_fixups(replay_file.packet_data.as_slice(), version);
    ...
    match format {
        ...
        ResultsFormat::Normalized => {
            // Constants only feed raw-results resolution inside `from_battle_report`.
            // Lesta replays embed no raw server results, so the normalized report is
            // built purely from packet-derived data (roster, observed damage, kills,
            // outcome) and needs no constants — don't require or fetch them then.
            let constants = if report.battle_results().is_some() {
                resolve_constants(stem, explicit_constants, resolver, &version, allow_approximate_constants)?
            } else {
                explicit_constants.cloned().unwrap_or_else(|| Value::Object(serde_json::Map::new()))
            };
            let mut normalized =
...
fn build_battle_report(
    ...
    // Lesta («Мир кораблей»): no onArenaStateReceived — seed the entity↔player
    // index from the in-stream roster blob (no-op for WG replays).
    world.seed_lesta_player_index(replay_file.packet_data.as_slice());
