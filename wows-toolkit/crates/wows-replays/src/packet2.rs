...
impl PacketTypeId {
    ...
    fn from_raw_modern(raw: u32) -> Self {
        match raw {
            ...
            // WG emits NestedPropertyUpdate at 0x23; Lesta («Мир кораблей», 26.x)
            // relocated it to 0x24 (0x23 goes unused there, and 0x24 is an unused
            // gap for WG). Accept both so team-score deltas — which ride on
            // BattleLogic NestedPropertyUpdates — decode for either client. Any
            // stray non-NPU 0x24 fails the format checks in the parser and is
            // dropped, so this is safe without version gating.
            0x23 | 0x24 => Self::NestedPropertyUpdate,
            0x25 => Self::Camera,
