...
impl ChatLoggerBuilder {
    ...
    pub fn build(self, meta: &crate::ReplayMeta, packet_data: &[u8]) -> Box<dyn Analyzer> {
        let version = Version::from_client_exe(&meta.clientVersionFromExe);
        // Lesta («Мир кораблей») replays carry no `onArenaStateReceived`, so the
        // usual sender→name seeding never fires. Pre-seed from the roster blob
        // (accountId + avatar id → name); no-op for WG replays, which still fill
        // the map from arena state during `process`.
        let mut usernames = HashMap::new();
        for (id, name) in crate::lesta_roster::chat_usernames(packet_data, &version) {
            usernames.insert(AccountId::from(id), name);
        }
        Box::new(ChatLogger {
            usernames,
