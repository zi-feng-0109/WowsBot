...
pub struct ReplayMeta {
    ...
    //mapBorder: Option<...>,
    pub duration: u32,
    /// Absent in Lesta («Мир кораблей») replays.
    #[serde(default)]
    pub gameLogic: Option<String>,
    pub name: String,
    pub scenario: String,
    pub playerID: AccountId,
    /// Absent in Lesta («Мир кораблей») replays — the roster is only in the packet stream there.
    #[serde(default)]
...
impl ReplayFile {
    ...
    pub fn from_bytes(bytes: &[u8]) -> rootcause::Result<ReplayFile, ParseError> {
        ...
        let mut replay = ReplayFile { meta: result.meta, raw_meta: result.raw_meta.to_string(), packet_data };
        // Lesta («Мир кораблей») carries no metadata `vehicles` roster; synthesize
        // one from the in-stream roster blob so every downstream consumer works
        // through the normal WG path. No-op for WG replays (roster already present).
        if replay.meta.vehicles.is_empty() {
            let version = wowsunpack::data::Version::from_client_exe(&replay.meta.clientVersionFromExe);
            replay.meta.vehicles =
                crate::lesta_roster::synthesize_vehicles(&replay.packet_data, &replay.meta.playerName, &version);
        }
        Ok(replay)
    }
