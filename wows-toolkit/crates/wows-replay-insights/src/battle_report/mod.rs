...
pub struct MatchMetadata {
    ...
    /// Per-team final scores (from the packet-derived BattleLogic teamsScore).
    /// `(team_index, score)`. Empty if unknown.
    pub team_scores: Vec<(usize, i64)>,
}
...
pub struct NormalizedPlayer {
    ...
    /// Packet-derived max/final HP (for the HP bar). `0` when unknown.
    pub max_health: f32,
    pub final_health: f32,
    /// Packet-derived death cause (e.g. `ApShell`, `Torpedo`, `Fire`) and killer
    /// player name. `None` when the ship survived or the info isn't in the stream.
    pub death_cause: Option<String>,
    pub killer_name: Option<String>,
    /// Self-player damage dealt to the enemy, grouped by weapon/source kind
    /// (`MainAp`, `Torpedo`, `Burn`, `Flood`, `TBomber`, ...) → total. Packet-
    /// derived from `receiveDamageStat`; empty for non-self. Feeds the review
    /// image's "output damage" donut (the received-damage donut is server-only
    /// and unavailable for Lesta).
    pub self_damage_by_type: Vec<(String, u64)>,
    /// Self-player received damage grouped by the source (attacker) ship name →
    /// total. Packet-derived (DamageReceived); no damage-type split is available,
    /// so this is "who hit me", not "by weapon type". Empty for non-self.
    pub self_received_by_source: Vec<(String, u64)>,
}
...
impl NormalizedBattleReport {
    pub fn from_battle_report(
        ...
        fill_self_ribbons_achievements(&mut players, report, provider);
        ...
        let metadata = MatchMetadata {
            ...
            team_scores: report.team_scores().iter().map(|ts| (ts.team_index, ts.score)).collect(),
        };
...
/// Fill the self player's ribbons/achievements from packet-derived self stats
/// when the server-JSON path left them empty. WG replays embed the results JSON
/// so these are already populated (all players) and this is a no-op; Lesta
/// replays embed no JSON, but the recording player's ribbons (`onRibbon` /
/// `privateVehicleState.ribbons`) and achievements (`onAchievementEarned`) are in
/// the packet stream and captured on `BattleReport::self_*`. Self-only by nature.
fn fill_self_ribbons_achievements(
    players: &mut [NormalizedPlayer],
    report: &BattleReport,
    provider: &GameMetadataProvider,
) {
    // Self received damage by source ship (packet-derived; who hit me). Resolve
    // each aggressor entity to its localized ship name via the report's players.
    let received_by_source: Vec<(String, u64)> = report
        .self_received_by_source()
        .iter()
        .map(|(ent, total)| {
            let label = report
                .players()
                .iter()
                .find(|p| p.initial_state().entity_id() == *ent)
                .and_then(|p| provider.localized_name_from_param(p.vehicle()))
                .unwrap_or_else(|| "环境".to_string());
            (label, *total as u64)
        })
        .collect();

    let Some(selfp) = players.iter_mut().find(|p| p.is_self) else {
        return;
    };
    if selfp.self_received_by_source.is_empty() {
        selfp.self_received_by_source = received_by_source;
    }

    if selfp.ribbons.is_empty() && !report.self_ribbons().is_empty() {
        let mut ribbons: Vec<RibbonResult> = report
            .self_ribbons()
            .iter()
            .filter_map(|(ribbon, count)| {
                let count = *count;
                if count == 0 {
                    return None;
                }
                let key = ribbon.translation_key()?;
                let t = wowsunpack::game_params::translations::translate_ribbon(key, provider)?;
                Some(RibbonResult {
                    name: key.to_string(),
                    display_name: t.display_name,
                    description: t.description,
                    icon_key: t.icon_key,
                    is_subribbon: t.is_subribbon,
                    count: count as u64,
                })
            })
            .collect();
        ribbons.sort_by(|a, b| a.name.cmp(&b.name));
        selfp.ribbons = ribbons;
    }

    if selfp.achievements.is_empty() && !report.self_achievements().is_empty() {
        let mut list: Vec<(i64, AchievementResult)> = report
            .self_achievements()
            .iter()
            .filter_map(|(id, count)| {
                ...
                let gp = <GameMetadataProvider as GameParamProvider>::game_param_by_id(provider, (id as u32).into())?;
                let ParamData::Achievement(data) = gp.data() else {
                    return None;
                };
                let ui_name = data.ui_name().to_string();
                let display_name =
        ...
        selfp.achievements = list.into_iter().map(|(_, a)| a).collect();
    }
}
...
fn build_player(
    ...
    let is_test_ship = vehicle_param.data().vehicle_ref().map(|vehicle| vehicle.is_test_ship()).unwrap_or_default();
    ...
    // Self-player damage dealt to enemies, grouped by weapon/source kind — feeds
    // the review "output damage" donut. Packet-derived, self-only.
    let self_damage_by_type: Vec<(String, u64)> = if is_self {
        let mut map: std::collections::BTreeMap<String, u64> = std::collections::BTreeMap::new();
        for e in report.self_damage_stats() {
            if e.category.known() == Some(&DamageStatCategory::Enemy)
                && let Some(w) = e.weapon.known()
                && e.total > 0.0
            {
                *map.entry(format!("{w:?}")).or_default() += e.total as u64;
            }
        }
    ...
    // Packet-derived HP + death info (available even for Lesta, which has no
    // server results). Killer is resolved to a player name via the entity id.
    let max_health = vehicle.map(|v| v.props().max_health()).unwrap_or(0.0);
    let final_health = vehicle.map(|v| v.props().health()).unwrap_or(0.0);
    let death = vehicle.and_then(|v| v.death_info());
    // Emit the bare cause name ("CsShell", "Fire", ...) so the renderer's
    // death-cause→Chinese map matches; unwrap the `Known(..)` wrapper.
    let death_cause = death.map(|d| match d.cause().known() {
        Some(c) => format!("{c:?}"),
        None => format!("{:?}", d.cause()),
    });
    let killer_name = death.and_then(|d| {
        let kid = d.killer();
        report
            .players()
            .iter()
            .find(|p| p.initial_state().entity_id() == kid)
            .map(|p| p.initial_state().username().to_string())
    });
    ...
    NormalizedPlayer {
        ...
        killer_name,
        self_damage_by_type,
        self_received_by_source: Vec::new(), // filled for the self player in fill_self_*
    }
}
