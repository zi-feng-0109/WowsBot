...
/// Record a self-player achievement (`onAchievementEarned`). `id` is the
/// achievement's GameParams id; `count` is how many were earned in this event.
/// Self-only, so we accumulate directly into `SelfStats.achievements`.
pub fn handle_achievement(id: i64, count: i64, world: &mut World) {
    let mut self_stats = world.resource_mut::<SelfStats>();
    *self_stats.achievements.entry(id).or_insert(0) += count;
}
