...
pub struct BattleReport {
    ...
    /// Self-player ribbon totals (from `onRibbon`/`privateVehicleState`). Self-only.
    self_ribbons: HashMap<Ribbon, usize>,
    /// Self-player achievement totals by GameParams id (from `onAchievementEarned`).
    self_achievements: HashMap<i64, i64>,
    /// Self-player received damage grouped by aggressor entity `(aggressor, total)`,
    /// sorted desc. Packet-derived (DamageReceived); no damage-type split exists.
    self_received_by_source: Vec<(EntityId, f32)>,
...
impl BattleReport {
    ...
    /// Self-player ribbon totals from the packet stream. Self-only (packets carry
    /// only the recording player's ribbons). Empty when none were captured.
    pub fn self_ribbons(&self) -> &HashMap<Ribbon, usize> {
        &self.self_ribbons
    }

    /// Self-player achievement totals by GameParams id, from `onAchievementEarned`.
    /// Self-only. Empty when none were captured.
    pub fn self_achievements(&self) -> &HashMap<i64, i64> {
        &self.self_achievements
    }

    /// Self-player received damage grouped by aggressor entity `(aggressor, total)`,
    /// sorted descending. Packet-derived; the source is the ship that dealt it
    /// (no damage-type breakdown exists in the replay stream).
    pub fn self_received_by_source(&self) -> &[(EntityId, f32)] {
        &self.self_received_by_source
    }
...
impl<'res, 'replay, G: ResourceLoader> BattleWorld<'res, 'replay, G> {
    ...
    pub fn into_report(mut self) -> BattleReport {
        ...
        let self_ribbons = self.world().resource::<SelfStats>().ribbons.clone();
        let self_achievements = self.world().resource::<SelfStats>().achievements.clone();

        // Self player's received damage grouped by the aggressor entity that dealt
        // it (from DamageReceived packets — packet-derived, so available for Lesta).
        // No damage-type breakdown exists in the stream, but "who hit me" does.
        let self_entity = self_player.initial_state().entity_id();
        let self_received_by_source: Vec<(EntityId, f32)> = {
            let ledger = self.world().resource::<DamageLedger>();
            let mut per_aggressor: HashMap<EntityId, f32> = HashMap::new();
            for (aggressor, events) in ledger.0.iter() {
                let total: f32 = events.iter().filter(|e| e.victim == self_entity).map(|e| e.amount).sum();
                if total > 0.0 {
                    per_aggressor.insert(*aggressor, total);
                }
            }
            let mut v: Vec<(EntityId, f32)> = per_aggressor.into_iter().collect();
