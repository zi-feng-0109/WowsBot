...
impl<'res, 'replay, G: ResourceLoader> BattleWorld<'res, 'replay, G> {
    ...
    /// Seed the entity↔player index from the Lesta («Мир кораблей») roster blob.
    /// WG builds this from `onArenaStateReceived`, which Lesta doesn't send; instead
    /// the roster (with ship entity ids) lives in an in-stream blob. Requires the
    /// MetadataPlayers roster already seeded (from the synthesized meta.vehicles).
    /// No-op if the index is already populated or no Lesta roster is present.
    pub fn seed_lesta_player_index(&mut self, packet_data: &[u8]) {
        if !self.world.resource::<PlayerIndex>().0.is_empty() {
            return;
        }
        let states = wows_replays::lesta_roster::extract_lesta_player_states(packet_data, &self.version);
        if states.is_empty() {
            return;
        }
        ...
        crate::ingest::entities::seed_vehicles_from_arena_state(
            states.iter(),
            GameClock(0.0),
            &mut self.world,
            self.resources,
