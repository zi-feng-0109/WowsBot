...
impl GameMetadataProvider {
    ...
    pub fn from_vfs(vfs: &vfs::VfsPath) -> Result<GameMetadataProvider, GameDataError> {
        ...
        // WG: content/GameParams.data。Lesta («Мир кораблей») 拆成 GameParams_py2/py3.data。
        // 按序回退,py2 pickle 跟现有解析器兼容,优先。
        let candidates = [
        ...
        for c in candidates {
            if let Ok(mut f) = vfs.join(c).and_then(|p| p.open_file()) {
                f.read_to_end(&mut game_params_data)?;
                opened = true;
                break;
            }
        }
