...
impl PkgFileLoader {
    ...
    pub fn read<P: AsRef<Path>, W: Write>(
        ...
        let raw = &mmap[start_offset..end_offset];
        // size==unpacked_size = 未压缩 (stored)。WG 用 compression_info==0 表示,
        // Lesta («Мир кораблей») 用非0 也可能是 stored,故以大小相等为准,两服通吃。
        let is_stored = file_info.compression_info == 0 || file_info.size == file_info.unpacked_size;
        if !is_stored {
            let mut decoder = DeflateDecoder::new(Cursor::new(raw));
            std::io::copy(&mut decoder, out_data)?;
        } else {
            std::io::copy(&mut Cursor::new(raw), out_data)?;
        }
