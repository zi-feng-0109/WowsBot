    ...
    T: Prime + Debug + Send + Sync + 'static,
{
    ...
    fn open_file(&self, path: &str) -> vfs::VfsResult<Box<dyn vfs::SeekAndRead + Send>> {
        ...
        // WG 约定 compression_info==0 = 未压缩;Lesta («Мир кораблей») 改了该字段编码
        // (用非0 如 6 也可能是 stored)。用 size==unpacked_size 判断"是否真未压缩" —— deflate
        // 必然改变大小,这个判据两服通吃。
        let is_stored = file_entry.compression_info == 0 || file_entry.size == file_entry.unpacked_size;
        if !is_stored {
