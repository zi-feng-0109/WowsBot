...
pub fn translations_path(game_dir: &Path, build: u32) -> PathBuf {
    let base = game_dir.join("bin").join(build.to_string());
    // WG installs ship-name translations under `en`; Lesta («Мир кораблей») ships
    // only `ru`, and community Chinese packs (e.g. 泽刻汉化) override `res_mods`.
    // Probe res_mods first (mod/汉化 overrides win), then res, over the locales we
    // know how to consume; return the first that exists so a Chinese pack, an
    // English WG client, and a bare Lesta (Russian) client all resolve.
    for root in ["res_mods", "res"] {
        for locale in ["en", "zh_sg", "zh", "zh_cn", "ru"] {
            let candidate = base.join(root).join("texts").join(locale).join("LC_MESSAGES/global.mo");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    // Fallback keeps the original "not found" warning path for callers.
    base.join("res/texts/en/LC_MESSAGES/global.mo")
}
