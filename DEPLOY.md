...
# 战报 + 复盘合并图（推荐 QQ 机器人直接用这个）
./bin/wows_full_report /tmp/某个真实回放.wowsreplay
# 预期：stdout 最后一行 = /tmp/wows_report/<回放名>.full.png
```

三个命令共用同一份 replayshark / specs / data，后两个不需要额外装东西。
...
三个命令 I/O 完全一致：参数 `<replay> [out_dir]`、stdout 最后一行是 PNG 路径、stderr 是进度日志、退出码非 0 表示失败。所以一个通用 helper 就够了。
...
async def make_report(kind: str, replay_path: str,
    ...
    cmd = {
        ...
        "full":   f"{BOT_HOME}/bin/wows_full_report",
    }[kind]
...
result = subprocess.run(
    [f"{BOT_HOME}/bin/wows_full_report", replay_path, out_dir],
...
> 性能小贴士：
> - `wows_damage_report` 内部会跑一次 `wows_report` 拿 JSON。这个 JSON 会缓存到
>   `<out_dir>/<回放名>.json`，只要它的 mtime ≥ 回放本身就直接复用，不会重复解析。
>   所以"先调 `wows_report` 再调 `wows_damage_report`"只会解析一次 replay。
> - `wows_full_report` 的中间产物（紧凑版战报 PNG、无 header 复盘 PNG、JSON）
>   全部走临时目录，最终只把 `<回放名>.full.png` 写到 `out_dir`，不会污染独立调用的
>   `<回放名>.png` / `<回放名>.damage.png`。如果你既要合并图、又要两张独立大图，
>   就分别再调一次 `wows_report` 和 `wows_damage_report`。
