...
> ⚠️ **`report/data/builds.json` 是 git 跟踪的生成产物,仓库里那份会过期。**
> 它由上面这个脚本从当前 `report/specs` 现生成;而更新流程(手工或 `/更新wg版本`)
> 只在**服务器上**重新生成,不会自动提交回仓库。所以仓库里那份是某次更新时的快照。
>
> 2026-09-20 实测踩到过:服务器上 `git reset --hard` 之后,builds.json 被倒回仓库里那份
> —— 而那份是 **15.3.0** 的,落后五个游戏版本,`/查询` 会查不到 15.4 之后新增的涂装与升级品。
> 症状很隐蔽:文件在、格式对、功能不报错,只是数据旧。
>
> **所以:在服务器上做过 `reset --hard` 或全新部署之后,一定要重跑一次
> `sudo python3 tools/build_builds_json.py`**,别假设仓库里那份是新的。
> 想彻底避免这个坑,可以把它从 git 里去掉(它本来就是生成产物),代价是全新部署在跑
> 生成脚本之前 `/查询` 的配装块走兜底。
