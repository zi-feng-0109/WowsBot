...
`/船 <中文舰名>` → **一次发出两张图**:现有数值卡(火力/鱼雷/生存/消耗品)+
**完整分面装甲图**。装甲图按装甲区(装甲核心 / 舰艏 / 舰艉 / 上层建筑 / 主炮塔 /
操舵室 / 防雷 / 船体)分组,每区逐个列出分面(舷侧 / 甲板 / 横向 / 倾斜…)和毫米
厚度,多层厚度并列显示。

> 注:最初设计是单独子指令 `/船 <名> 装甲`;后改为 `/船 <名>` 同时附带数值卡 +
> 装甲卡(2026-06-11,用户要求)。无装甲数据的船只发数值卡;装甲渲染失败也不影响
> 数值卡照常发出。
...
现有 `/船` handler:解析舰名 → 渲数值卡 → 若该船有装甲数据再渲装甲卡 →
两图合并成一条消息发出(无子参数)。(旧设计:**末位参数 == `装甲`** 分流,已废)
<!-- 旧描述保留供对照: -->
旧:若**末位参数 == `装甲`**,分流到装甲渲染,
